...
library(XML)
library(magrittr)
aurl <- "https://www.amazon.in/Stick-all-new-Remote-Streaming-Player/product-reviews/B0791YHVMK/ref=cm_cr_arp_d_paging_btm_next_2?ie=UTF8&reviewerType=all_reviews&pageNumber"
amazon_reviews <- NULL
for (i in 1:10){
...
write.table(amazon_reviews,"firesticks.txt")
getwd()
...
pos.words=c(pos.words,"wow", "kudos", "hurray","wow") 			# including our own positive words to the existing list
neg.words = c(neg.words,"disgusting","pathetic")        # including our own negative words to the existing list
stopwdrds = readLines(file.choose())

# Loading the text file to be analysed 
x = readLines(file.choose())
x[100]
library(corpus) # Preparing corpus from the text document 
library(tm)
...
x1 = tm_map(x1, removeWords, c(stopwords("english"),"amazon","remote","stick","fire", stopwdrds))
